...
from codescholar.sast.sast_utils import DropDecorators, CodeSpan, remove_comments_and_docstrings, collapse_nodes, label_nodes
import gast as ast
            ...
            if value is None or isinstance(value, AST_NODE_FILTER):
                ...
                for index, item in enumerate(value):
                    ...
                    if isinstance(item, ast.AST):
                        program_graph.add_new_edge(ast_node, item, pb.EdgeType.FIELD, list_field_name)
                    else:
                        item_node = make_node_from_ast_value(item)
                        program_graph.add_node(item_node)
                        program_graph.add_new_edge(ast_node, item_node, pb.EdgeType.FIELD, list_field_name)
